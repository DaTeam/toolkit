...
async function styleCopy({ from, to }) {
    if (!(await fse.exists(to))) {
        console.warn(`path ${to} does not exists`);
        return [];
    }

    const files = await glob('**/*.scss', { cwd: from });
    const cmds = files.map((file) => fse.copy(path.resolve(from, file), path.resolve(to, file)));
    return Promise.all(cmds);
}
...
async function run() {
    try {
        ...
        await styleCopy({ from: srcPath, to: buildPath });
        // await styleCopy({ from: srcPath, to: path.join(buildPath, './sass') });
